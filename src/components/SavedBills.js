...
import { Eye, Search, Calendar, IndianRupee, Trash2, Filter, CheckCircle2, Clock, Download, Zap, X } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div className="flex-1">
          ...
          {showBanner ? (
            <div className="mt-6 bg-blue-600/10 border border-blue-500/30 p-6 rounded-[2rem] relative overflow-hidden animate-in slide-in-from-left duration-500">
              <button
                onClick={() => setShowBanner(false)}
                className="absolute top-4 right-4 text-blue-400 hover:text-white transition-colors p-2 hover:bg-white/5 rounded-full"
                title="Minimise"
              >
                <X size={18} />
              </button>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-lg font-black text-white uppercase tracking-tighter">PLATFORM FOR SALE - ₹1,000</h4>
                  <p className="text-xs text-blue-300 font-bold uppercase tracking-widest">Contact @krish_root_labs on Instagram</p>
                </div>
              </div>
            </div>
          ) : (
            <button
              ...
              className="mt-4 flex items-center gap-3 bg-blue-600/20 border border-blue-500/30 px-5 py-2 rounded-full text-blue-400 hover:text-white transition-all group hover:scale-105 active:scale-95 animate-in fade-in duration-300"
            >
              <Zap size={14} fill="currentColor" className="animate-pulse" />
              <span className="text-[10px] font-black uppercase tracking-widest">Show Marketing Offer</span>
            </button>
          )}
        </div>
